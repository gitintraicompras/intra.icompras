		...
		<div class="table-responsive">
			<table id="idTable" class="table table-striped table-bordered table-condensed table-hover">
				...
				@foreach ($gruporen as $gr)
					...
					@endphp
					<tr>
						<td style="background-color: {{$backcolor}}; color: {{$forecolor}}; ">
							{{$loop->iteration}}
						</td>
						<td class="hidden-xs">
                            <div align="center">
						...
						<td>
							...
							<a href="{{URL::action('ConfigController@edit',$gr->codcli)}}">
						...
						<td style="padding-top: 10px;">
							...
							<center>
							@if($gr->status=="ACTIVO")
							    <input type="checkbox" id="idstatus_{{$gr->id}}_{{$gr->codcli}}" checked />
							@else
								<input type="checkbox" id="idpstatus_{{$gr->id}}_{{$gr->codcli}}"  />
							@endif
							</center>
							</span>
						</td>
						<td style="padding-top: 10px;">
							...
							<center>
							@if ($gr->predet==1)
							    <input type="checkbox" id="idmodpredet_{{$gr->id}}_{{$gr->codcli}}" 
							    checked />
							@else
								<input type="checkbox" id="idmodpredet_{{$gr->id}}_{{$gr->codcli}}"  />
							@endif
							...
							<div class="form-group">
				                <div class="input-group input-group-sm">
				                    ...
				                    <span class="input-group-btn BtnRestar">
				                        <button style="width: 50px;" type="button" class="btn btn-pedido" data-toggle="tooltip" title="Subir nivel de preferencia" id="idBtnRestar-{{$gr->codcli}}-{{$gr->preferencia}}">
				                        <span class="fa fas fa-angle-up" aria-hidden="true" id="idBtnRestar-{{$gr->codcli}}-{{$gr->preferencia}}"></span>
				                        </button>
				                    </span>

				                    <input id="idPref-{{$gr->preferencia}}" readonly value="{{$gr->preferencia}}" type="text" class="form-control"  style="color: #000000; text-align: center; width: 50px;" >

				                    <span class="input-group-btn BtnSumar">
				                        <button style="width: 50px;" type="button" class="btn btn-pedido" data-toggle="tooltip" title="Bajar nivel de preferencia" id="idBtnSumar-{{$gr->codcli}}-{{$gr->preferencia}}">
				                        <span class="fa fas fa-angle-down" aria-hidden="true" id="idBtnSumar-{{$gr->codcli}}-{{$gr->preferencia}}"></span>
				                        </button>
				                    </span>
						...
						@if ( date('d-m-Y', strtotime($feccatalogo)) == date('d-m-Y') )
							<td>
								{{date('d-m-Y H:i', strtotime($feccatalogo))}}
							</td>
						@else
							<td style="color: red;">
								{{date('d-m-Y H:i', strtotime($feccatalogo))}}
